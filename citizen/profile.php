...
$has_disability = (int)$user['has_disability'];
$disability_type = isset($_POST['disability_type']) ? trim($_POST['disability_type']) : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    $update_stmt = $conn->prepare("
        UPDATE users 
        SET name = ?, email = ?, phone = ?, disability_type = ?
        WHERE id = ?
    ");

    $update_stmt->bind_param("ssssi", $name, $email, $phone, $disability_type, $user_id);
...
<div class="content">
    ...
    <div class="profile-card">
        ...
        <div class="profile-email">
            <?php echo htmlspecialchars($user['email'] ?? $user['phone']); ?>
        </div>
    ...
    <div class="forms-section">
        ...
        <form method="POST" action="profile.php">
            ...
            <div class="form-group">
                <label>Age:</label>
                <input type="text"
                    value="<?= $age !== null ? $age . ' years' : 'N/A' ?>" disabled>
            </div>

            <?php if ($has_disability === 1): ?>
                <div class="form-group">
                    <label>Type of Disability:</label>
                    <input type="text" name="disability_type"
                        value="<?= htmlspecialchars($user['disability_type'] ?? '') ?>"
